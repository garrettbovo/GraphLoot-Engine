...
{
    //variable declarations and creating file for writing statistics
...
PathResult Graph::runDijkstra(const string &from, const string &to) const
{
    //dist stores the shortest known path from distance to each from node
    unordered_map<string, double> dist;
    //prev stores the previous node on the shortest path
    ...
    newRun.path.push_back(from);

    reverse(newRun.path.begin(), newRun.path.end());

    return newRun;
}

//method for showing fastest route from A*
PathResult Graph::runAStar(const string &from, const string &to) const
{
    //dist stores the shortest known path from distance to each from node
    unordered_map<string, double> gScore, fScore;
    //prev stores the previous node on the shortest path
